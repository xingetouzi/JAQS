...
def cal_rets_stats(df,period):
    ...
    ret_summary_table["skewness"] = scst.skew(df, axis=0)
    ret_summary_table["kurtosis"] = scst.kurtosis(df, axis=0)
    ret_summary_table['Ann. Ret'] = annual_ret
    ret_summary_table['Ann. Vol'] = annual_vol
    ret_summary_table['Ann. IR'] = annual_ret/annual_vol
    ret_summary_table['occurance'] = len(df)
    return ret_summary_table
...
def return_stats(signal_data,is_event,period):
    ...
    for ret_type in rets.keys():
        if len(rets[ret_type])>0:
            ret_stats = cal_rets_stats(rets[ret_type],period)
...
def get_rets(signal_data,is_event):
    ...
    if not is_event:
        ...
        rets['period_wise_tmb_ret'] = pfm.calc_return_diff_mean_std(period_wise_quantile_ret_stats[n_quantiles],
...
def analysis(signal_data,is_event,period):
    if is_event:
        return {"return":return_stats(signal_data,True,period)}
    else:
        return {
        "ic":ic_stats(signal_data),
        "return":return_stats(signal_data,False,period)
    }
